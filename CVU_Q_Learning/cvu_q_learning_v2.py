...
class AdaptativeLineDetector :
	...
	def learnHowToFollow(self,iteracitonLimit) :
		step = 0
		iteraction = 1
		for frame in self.camera.capture_continuous(self.rawCapture,format='bgr',use_video_port=True) :
			...
			if(step == 0) :
				...
				self.agent.learn(lineCenter,step)
				step = 1
			else :
				print "---------- NEXT ----------"
				lineCenter = self.getLineCenter(self.processNewFrame(frame))
				self.agent.learn(lineCenter,step)
				step = 0
				self.agent.MCU.stopMotors()
				self.agent.printQTable(iteraction)
				x = raw_input()

			self.rawCapture.truncate(0)
			iteraction = iteraction + 0.5
			if(iteraction > iteracitonLimit) : break
		self.agent.printQTable(iteraction)
...
agent = QLearning('Vision Q-learning',mcu,240,150,0.1,0.9,states,actions,rewards,CAM)
